...
### 10.5 Wynik i mapa liczone z demki przy dodawaniu wyniku — ✅ zaimplementowane
- `POST /api/results` przeszedł z JSON na `multipart/form-data` (te same role: Coach/Manager) — opcjonalne pole `demo` obok dotychczasowych pól formularza. Plik jest parsowany w pamięci i odrzucany, dokładnie jak w 10.4; nic nie ląduje na dysku ani w bazie.
- `DemoParseResult` dostał `Rounds` (`DemoRoundResult`: zwycięska strona + SteamID-y po stronie T i CT). Parser buduje je ze zdarzeń `PlayerTeam` (numer drużyny 2 = T, 3 = CT) i `RoundEnd` (`Winner` tak samo 2/3), bo demo samo w sobie nie wie, która drużyna jest „nasza".
- `DemoScoreCalculator` rozstrzyga przynależność **per runda**, nie raz na mecz: liczy, ilu graczy ze składu stoi po każdej ze stron i wybiera większość. Zmiana stron na połowie i w dogrywce wychodzi z tego sama, bez śledzenia numeru rundy.
- Ręcznie wpisany wynik jest teraz opcjonalny — służy jako fallback, gdy demki nie ma albo gdy żadnej rundy nie da się przypisać (realny przypadek dopóki skład nie uzupełni `SteamId64` w `/settings`). Gdy nie ma ani demki, ani wpisanego wyniku, handler zwraca `ResultErrors.ScoreRequired`, a frontend i tak blokuje wcześniej przycisk „Dodaj wynik".
- `Przeciwnik`, `Data meczu`, `Typ meczu` (z pickerem turnieju/ligi) i `Notatki` zostają w pełni ręczne. `DemoUrl` (link do demki hostowanej zewnętrznie) żyje dalej obok uploadu — to dwie różne rzeczy: link jest do pobrania przez drużynę, plik tylko do jednorazowego sparsowania.

### 10.6 Poprawki dokładności parsera demek — ✅ zaimplementowane
Porównanie wyników z HarnasHuba z zaufanym serwisem statystyk na tych samych demkach pokazało zawyżone ADR/Rating/liczby zabójstw i przesunięte kropki na mapie śmierci. Cztery niezależne przyczyny, wszystkie zweryfikowane empirycznie na prawdziwych demkach (Ancient / Anubis / Mirage):

- **Rozgrzewka i runda nożowa wchodziły do statystyk.** Parser liczył każdą rundę od pierwszego ticku pliku. Bramkowanie stoi teraz na `CCSGameRules.WarmupPeriod`/`HasMatchStarted`, czytanych **na żywo**, a nie na jednorazowym `warmup_end`/`round_announce_match_start` — nagranie może się zacząć po tych zdarzeniach i nigdy ich nie zobaczyć (ta sama filozofia co czytanie stron drużyn z `CSTeamNum`). Dwie pułapki, na które trzeba uważać przy zmianach tutaj: `HasMatchStarted` gaśnie **przed** `round_end` ostatniej rundy meczu (dlatego flaga jest zatrzaskiwana na `round_start` i obowiązuje całą rundę), a `round_announce_match_start` kasuje wszystko zebrane wcześniej, bo demka potrafi zawierać ogon poprzedniego meczu. Na demce z Anubisa dało to 20 → 19 rund i 141 → 132 zgonów.
- **`player_hurt.dmg_health` to obrażenia surowe, nie odjęte HP.** Strzał z AWP w klatę raportuje 115, headshot z Deagle grubo ponad 200 — na prawdziwej demce **20,6%** sumy obrażeń to był taki „overkill". Parser śledzi HP ofiary przed trafieniem (pole `Health` tego samego zdarzenia to HP **po** trafieniu, więc zasila kolejne) i przycina. Obrażenia od własnej drużyny nie wchodzą do ADR, tak jak w HLTV. Efekt na demce z Ancienta: średnie ADR drużyny 101 → 79 (realistyczne), Rating najlepszego gracza 1,72 → 1,60.
- **Pozycja zgonu bywała nieaktualna.** Zapamiętywana była tylko przy `player_hurt`, więc gracz, który po ostatnim trafieniu przeszedł kawałek i dopiero potem zginął (dopalenie molotovem, wykrwawienie, upadek), rysował się w starym miejscu — na demce z Anubisa 4 z 141 zgonów były przesunięte, jeden o **2813 jednostek**. Teraz pozycja odświeża się na `EntityEvents.CCSPlayerPawn.PostUpdate` dla każdego żywego gracza (koszt parsowania bez zmian, ~9 s na 335 MB) i jest czyszczona co rundę.
- **Obrazy radarów nie są oficjalnymi overview'ami 1024×1024.** Tylko `cache.webp` (1024×1024) i `anubis.webp` (2048×2048, czyli overview ×2) mają oryginalne kadrowanie — reszta to ciaśniejsze, nie-kwadratowe wycinki (np. `ancient.webp` 1290×1467). Ułamek policzony w przestrzeni overview lądował więc gdzie indziej po rozciągnięciu obrazu w przeglądarce — stąd zgon „obok okna" zamiast „w oknie". `MapCalibration.RadarImageCrops` przelicza ułamek na przestrzeń **wyświetlanego** obrazu, czyli tę samą, w której siedzą pinezki pozycji, granatów i taktyk. Kadry dopasowano numerycznie (maksymalizacja pokrycia chmury pozycji graczy z demki i geometrii na obrazie) i sprawdzono wzrokowo: trafialność kropek w geometrię mapy Ancient 63,7% → 87,2%, Mirage 78,5% → 97,7%. **Dust2, Inferno i Nuke są przycięte tak samo, ale nie było demki do dopasowania — czekają na nią**; dodanie wpisu do `RadarImageCrops` to cała robota.
