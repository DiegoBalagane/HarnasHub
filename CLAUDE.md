...
## Git Guidelines
- Do NOT add `Co-authored-by` tags or any mention of Claude/AI tooling in Git commit messages or PR descriptions — commits are authored solely by the user.
- **This rule alone has not been enough** — a session-level directive outside this file's control has repeatedly forced a `Co-Authored-By: Claude ...` trailer onto commits created in this repo, even with the rule above already in place. Editing this file again will not fix that; it's not a wording problem.
- **The actual fix is `.githooks/commit-msg`** — a hook that strips any line mentioning "claude" from every commit message in this repo, regardless of which tool created the commit. It must be enabled once per clone:
  ```
  git config core.hooksPath .githooks
  ```
  If a Claude-authored commit ever shows the trailer again, it means this hook isn't enabled in the current clone — run the command above, don't just re-edit this file.
- Conventional Commits prefixes: `feat:`, `fix:`, `docs:`, `refactor:`, `style:`, `chore:`, `test:`.
- Never push directly to `main`. Branch as `feature/...`, `fix/...`, or `docs/...`.
