...
public class StatsEndpoints : IEndpoint
{
    ...
    {
        var matchStats = app.MapGroup("/api/matches/{matchResultId:guid}/stats").WithTags("Stats").RequireAuthorization();
        ...
        var stats = app.MapGroup("/api/stats").WithTags("Stats").RequireAuthorization();
